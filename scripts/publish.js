...
(async () => {
  ...
  const result =
    await $`nx affected:libs --base=${BASE_ORIGIN} --exclude simple express blog --plain`;
  ...
  await $`nx affected:build --with-deps --base=${BASE_ORIGIN} --prod --parallel --exclude simple express blog`;
