import { InvalidDecoratorException } from './exception';

import { BaseController as Controller } from '@app/bases/controller';
import { Service } from '@app/bases/service';

// 前置处理
export type BeforeHandler<C extends Controller> = (
  instance: C,
 ...
 */
export const makeMethodDecoratorForController =
  <C extends Controller>(beforeHandler: BeforeHandler<C>) =>
 ...
 * @param {PropertyDescriptor} descriptor 成员的属性描述符
 */
export const makeMethodDecoratorForControllerAndInjectService = <
  C extends Controller & { service: Service }
>(
 ...
 * @param afterHandler
 */
export const makeAfterMethodDecoratorForController =
  <C extends Controller>(afterHandler: BeforeHandler<C>) =>
  (target: any, key: string, descriptor: PropertyDescriptor) => {
    const originFunction = descriptor.value;
 ...
 * @param afterHandler
 */
export const makeAfterMethodDecoratorForControllerAndInjectService = <
  C extends Controller & { service: Service }
>(
  beforeHandler: BeforeHandler<C>
) =>
